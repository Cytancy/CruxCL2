...
$(document).ready(function() {
	var config = {
		...
		components = {
			...
			slideA: {
				...
				tagline: {
					element: $(".clc-slide-a-tagline"),
					items: {
						element: $(".clc-slide-a-tagline-item"),
					}
				},
				content: {
					background: {
	...
	function setupEntryCover() {
		...
		function enter(lock) {
			...
			entryTimeline.from(components.slideA.content.background.element, .38, {
				opacity: .16,
			}, 1.9);
	...
	function setupSlideJ() {
		...
		function setupActivation() {
			...
			function updateActivation() {
				if (screen.activeSlide.number == 4) {
					activate();
				}
				else if (!screen.visibleSlides["4"]) {
					deactivate();
				}
			}
